...
$sql = mysqli_query($conn,"SELECT * FROM tb_event WHERE id='$id' ");
$row = mysqli_fetch_assoc($sql);
$tanggal = date_create($row['tanggal']) ;
$tanggal = date_format($tanggal,'jS F Y' );
?><div id="lokasi"></div>
	<div id="fh5co-featured-menu" class="fh5co-section">
		<div class="container" >
			<div class="row" >
				<div class="col-md-12 fh5co-heading animate-box" style="margin-bottom: 10px; ">

					<h2>Event <?php echo $row['nama']; ?>.</h2>
					<h2 style="font-size: 1em;"><?php echo $tanggal ?>.</h2>
					<div class="row" >
					...
					<?php
					$tanggal = date_create($row['tgl_berlaku']) ;
					$tanggal = date_format($tanggal,'jS F Y' );
					$tanggal2 = date_create($row['tgl_berakhir']) ;
					$tanggal2 = date_format($tanggal2,'jS F Y' );
					$paymentDate = date('Y-m-d');
    				$paymentDate=date('Y-m-d', strtotime($paymentDate));;
    				$contractDateBegin = date('Y-m-d', strtotime($row['tgl_berlaku']));
   					$contractDateEnd = date('Y-m-d', strtotime($row['tgl_berakhir']. ' +1 day'));
   					if (($paymentDate > $contractDateBegin) && ($paymentDate < $contractDateEnd)){

   						?>
   						<h2 style="font-size: 1em;">Berlaku Event</h2>
						<h2 style="font-size: 1em;color: #e74c3c;"><?php echo $tanggal; ?> <font style="color: white;">Sampai </font><?php echo $tanggal2; ?></h2>
   						<?php
   					}else{
   						?>
   						<h2 style="font-size: 1em;color: #e74c3c;">EVENT SUDAH BERAKHIR ..</h2>
   						<?php
   					}
   					$view= $row['view'] + 1;
   					$sql = "UPDATE tb_event SET  `view` ='$view' WHERE id = $id";
					$result = mysqli_query($conn, $sql);
					?>
				...
				<div class="col-md-3 col-sm-6 col-xs-6 col-xxs-12 fh5co-item-wrap" style="    width: 100%;">
				

					<div class="fh5co-item animate-box"  style="    width: 100%;">
						<img src="upload/<?php echo $row['gambar']; ?>" class="img-responsive" alt="<?php echo $row['nama']; ?>">
